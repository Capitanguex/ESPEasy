  ...
  switch (function)
  {
    ...
    case PLUGIN_WEBFORM_LOAD:
    {
...

      {
        ...
        lsHtml += F("> Running Lights (Random Color) &nbsp;&nbsp;&nbsp;");
        lsHtml += F("<input type='radio' name='lightshow_mode' value='2'");
        if (PCONFIG(7) == P070_LIGHTSHOW_FLASH) { lsHtml += F(" checked"); }
        lsHtml += F("> Flash (all LEDs + Pointer-Blinks)");
        addRowLabel(F("Lightshow Mode"));
        addHtml(lsHtml);
      }
      addFormNote(F("The Lightshow starts every full hour (hh:00:00)"));
